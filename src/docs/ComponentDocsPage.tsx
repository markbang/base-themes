...
import { type ComponentGroup } from './componentMeta'
import { navigateTo, toComponentPath } from './routing'
...
const BUG_REPORT_URL = `${PROJECT_REPO_URL}/issues/new?template=bug_report.yml`
const GALLERY_SUBMISSION_URL = `${PROJECT_REPO_URL}/issues/new?template=gallery_submission.yml`
const BASE_UI_DOCS_URL = 'https://base-ui.com'
...
function useComponentDocs(): ComponentDoc[] {
  ...
  const navItems = [
    { label: t.nav.components, href: toComponentPath('button') },
    {
      ...
      children: [
        { label: t.extended.docs, href: BASE_UI_DOCS_URL },
        { label: t.extended.github, href: PROJECT_REPO_URL },
        { label: t.extended.releases, href: PROJECT_RELEASES_URL },
      ],
    },
...
function trackComponentFeedbackClick(componentId: string, target: string) {
  trackEvent('github_outbound_click', { source: 'component-docs', component: componentId, target })
}

function ComponentFeedbackCta({ doc }: { doc: ComponentDoc }) {
  return (
    <section className="doc-feedback-cta" aria-labelledby={`${doc.id}-feedback-title`}>
      <div>
        <div className="doc-kicker">After trying it</div>
        <h2 id={`${doc.id}-feedback-title`}>Turn this component into a public signal</h2>
        <p>Use {doc.title} in a real screen, then leave the smallest useful signal: star or fork the repo, discuss what worked, request the missing variant, report an integration bug, or submit a gallery example.</p>
      </div>
      <div className="doc-feedback-actions">
        <a href={PROJECT_REPO_URL} target="_blank" rel="noreferrer" onClick={() => trackComponentFeedbackClick(doc.id, 'repo-star')}>
          <Star size={17} /> Star
        </a>
        <a href={PROJECT_FORK_URL} target="_blank" rel="noreferrer" onClick={() => trackComponentFeedbackClick(doc.id, 'repo-fork')}>
          <GitFork size={17} /> Fork
        </a>
        <a href={SHOW_AND_TELL_URL} target="_blank" rel="noreferrer" onClick={() => trackComponentFeedbackClick(doc.id, 'show-and-tell')}>
          <MessageCircle size={17} /> Discuss
        </a>
        <a href={FEATURE_REQUEST_URL} target="_blank" rel="noreferrer" onClick={() => trackComponentFeedbackClick(doc.id, 'feature-request')}>
          <MessageSquarePlus size={17} /> Request
        </a>
        <a href={BUG_REPORT_URL} target="_blank" rel="noreferrer" onClick={() => trackComponentFeedbackClick(doc.id, 'bug-report')}>
          <MessageSquarePlus size={17} /> Bug
        </a>
        <a href={GALLERY_SUBMISSION_URL} target="_blank" rel="noreferrer" onClick={() => trackComponentFeedbackClick(doc.id, 'gallery-submission')}>
...
function ComponentPage({ doc }: { doc: ComponentDoc }) {
  ...
  return (
    <article className="component-page">
      ...
      <ComponentFeedbackCta doc={doc} />
    </article>
  )
}

export default function ComponentDocsPage({ activeId }: { activeId: string }) {
  const docs = useComponentDocs()
  const doc = docs.find((item) => item.id === activeId) ?? docs[0]

  return (
    <>
      <Sidebar docs={docs} activeId={doc.id} />

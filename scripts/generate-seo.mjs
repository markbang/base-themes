...
function renderLlmsTxt() {
  ...
  const linkList = (links) => links.map(([label, path]) => `- [${label}](${absoluteUrl(path)})`).join('\n')
...
function renderLlmsFullTxt(pages) {
  const pageLinks = pages.map((page) => `- [${page.title}](${absoluteUrl(page.path)}): ${page.description}`).join('\n')
  const componentLinks = componentMeta.map((component) => `- [${component.title}](${absoluteUrl(`/components/${component.id}`)}): registry name \`${component.registryName}\`; group ${component.group}; ${component.summary}`).join('\n')
  const themeLinks = themeMeta.map((theme) => `- [${theme.label}](${absoluteUrl(`/themes/${theme.style}`)}): style \`${theme.style}\`; ${theme.description}`).join('\n')
  const blockLinks = blockMeta.map((block) => `- [${block.title}](${absoluteUrl(`/blocks/${block.id}`)}): registry name \`${block.registryName}\`; export \`${block.exportName}\`; category ${block.category}; ${block.description}`).join('\n')
  const registryItemLinks = registry.components
    .slice(0, 12)
    .map((component) => `- [${component.title}](${absoluteUrl(`/registry/items/${component.name}.json`)})`)
    .join('\n')
  const blockItemLinks = registry.blocks.map((block) => `- [${block.title}](${absoluteUrl(`/registry/items/block-${block.name}.json`)})`).join('\n')
  const themeItemLinks = themeMeta.slice(0, 8).map((theme) => `- [${theme.label}](${absoluteUrl(`/registry/items/theme-${theme.style}.json`)})`).join('\n')
...
function getPageType(page) {
  return page.path.startsWith('/components/') || page.path.startsWith('/themes/') || page.path.startsWith('/docs/') ? 'article' : 'website'
}
...
function injectSeo(html, page) {
  ...
  return html
    ...
    .replace(/<meta property="og:image:alt" content="[^"]*" \/>/, `<meta property="og:image:alt" content="${escapeHtml(page.title)} preview" />`)
    ...
    .replace(/<script type="application\/ld\+json" id="structured-data">[\s\S]*?<\/script>/, `<script type="application/ld+json" id="structured-data">${structuredData}</script>`)
}
...
async function writeRegistryArtifacts() {
  ...
  await Promise.all([
    copyFile(resolve('registry/registry.json'), resolve(registryDir, 'registry.json')),
    copyFile(resolve('registry/shadcn-registry.json'), resolve(registryDir, 'shadcn-registry.json')),
    cp(resolve('registry/items'), resolve(registryDir, 'items'), { recursive: true }),
    copyFile(resolve('src/docs/blockMeta.json'), resolve(registryDir, 'block-meta.json')),
    copyFile(resolve('src/docs/componentMeta.json'), resolve(registryDir, 'component-meta.json')),
    copyFile(resolve('src/docs/staticPageMeta.json'), resolve(registryDir, 'static-page-meta.json')),
    copyFile(resolve('src/docs/themeMeta.json'), resolve(registryDir, 'theme-meta.json')),
  ])
}
